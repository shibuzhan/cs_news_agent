...
- 用户指令：①贴出公众号草稿截图，指出“上传的封面图根本不是项目截图，而是之前的文章插图”；②追问“为什么把 AI 图全排除？是不是提示词问题？优先用真实图，残缺位置用 AI 图补充”。
- 影响范围：`app/tools/illustration_planner.py`（来源判定、选择口径、确定性策略）、`app/agent_tools/source_media_tools.py`（实时 README 兜底核对、素材名 `source-` 前缀）、`tests/test_inline_illustration_floor.py`、`tests/test_image_generation.py`；重建 app/worker。**用到 1 次配图规划模型调用与微信素材上传（真实投递覆盖）**。
- 根因①（封面为何不是项目截图）：草稿里的"封面"一直是 AI 生成的图，真实截图从未被绑成封面——`attach_source_image` 之前只被我当正文图用过。
- 根因②（GitHub API 配额）：抓实时 README 时报 `rate_limited`（匿名 60 次/小时被今晚排查用尽）→ 改为用 `raw.githubusercontent.com` 取 README（不吃 API 配额）。
- 根因③（候选清单过旧）：`attach_source_image` 的允许清单只来自**存储版** README（缺 `docs/media` 两张官方截图）→ 增加实时 README 兜底核对，来源范围不放宽（仍只允许仓库/官方页面）。
- 根因④（AI 图为何被整批排除）：上一轮提示词写了“与文章主题无关的才排除”，而 AI 配图是氛围图，模型据此**清空**了正文插图；这是提示词给了过宽的排除许可。
- 根因⑤（来源判定失效）：`_is_source_asset` 原先按素材名判断，而插图行上没有该字段 → 改为按 **`provider`** 判断（AI 图有供应商 `agnes`，真实素材为 NULL）。
- 根因⑥（改了策略却不见效）：投递记录**已有素材选择时不会重新选择**（`cover_media_id` 存在即复用），所以“刷新投递”只是拿旧选择重新渲染；必须作废选择（`delivery_stale`）才会真正重选。
- 处理结论（口径落地为确定性规则）：提示词改为“真实截图（origin=source）优先，AI 配图补足 2–3 张，**不得因主题不完全对应就整批排除**”；并新增 `_apply_selection_policy()`——保留模型选中的真实截图 → **强制包含其余真实截图**（非封面）→ 保留模型选中的 AI 图 → 不足 2 张时按段位补 AI 图，最多 6 张。
- 实测（最终状态）：封面 `youtube-popular-videos.png`（官方截图）；正文插图 `source-open-source-survey.png`（官方截图，排第一）+ 2 张 AI 配图；远端草稿 `lJDDFrVGnUHQ` **原地覆盖**、未新建。
- 验证：后端 **283 项通过、0 失败**（新增策略与来源判定用例）；容器内实测选择策略生效。
- 遗留：①“已投递且已有选择时不重选”是既有设计，改口径/换图后需要先作废选择才会生效——是否要在配置或口径变更时自动作废，待用户决定；②`list_source_images` 仍按用户要求以存储版 README 为准，实时截图需要显式指定（现已能绑定）。
- 授权状态：已确认并完成
...
- 用户指令：“你的字数是怎么算的？为什么我这看才2000不到？”→ 追问后澄清：“这篇稿子先不动，**问题在于你写的时候保留的空行上传微信草稿箱后没有空行了**”。
- 影响范围：`app/services/wechat_official.py`（`PARAGRAPH_STYLE` / `IMAGE_PARAGRAPH_STYLE` / `_paragraph()`）、`tests/test_wechat_official_account.py`、`tests/test_auto_review.py`。
- 结论①（字数口径，先给事实）：同一篇 v4 草稿按不同口径分别是——`len(body)` 2628；去掉 16 个段首全角缩进 2612；**规则审核用的"正文长度" 2614**（去来源尾注后原样数，含缩进）；**成形校验 `body_chars` 2483**（`compose_natural_article` 去所有空白）；汉字 1657、汉字+英数词 1776；公众号编辑器显示 1957。根因是**提示词与校验说的不是同一个东西**：生成/改稿提示词写的是"1600 到 2200 个**中文字符**"，而校验数的是去空白后的**全部字符**（含英文字母与标点），两者对同一篇稿子会得出相反结论（汉字 1657＝目标带下限，总字符 2483＝超硬上限 2400，v4 的规则审核正是因这一条 `passed=False`）。用户选择先按"统一口径"方向推进（方案 A），本轮未改判定逻辑。
- 结论②（空行消失的真实根因）：`render_wechat_html` 此前只发 `<p>正文</p>`，**段间距完全交给微信编辑器默认值**，草稿编辑器里相邻段落会贴在一起，看起来就像"空行被吃了"；行高也只有 1.0。这不是内容里的空行丢了（`body` 里 9 段以空行分隔、渲染出 9 个 `<p>` 都正确），而是**段间距没有随内容一起发出去**。
- 处理结论：每个段落改为带内联样式——正文 `margin:0 0 22px;line-height:1.75`，图片段落 `margin:0 0 12px;line-height:0`。内联样式写在 content 字段里，草稿编辑器与发表后的正文都会照它渲染，两端观感一致。
- 验证（单测 + 只读实测，**未跑生成/审核/检索外呼**）：后端 **442 项通过、0 失败**（新增 1 项段间距断言，另 2 项断言从字面 `<p>` 改为匹配段落文本）；app/worker 已重建；容器内确认渲染输出带样式（`<p style="margin:0 0 22px;line-height:1.75;">　　第一段</p>`）。只读探针 `draft/batchget` + `draft/get` 显示**当前公众号草稿箱为空**（库里的旧 media_id 已失效，返回 40007），因此没能读回历史草稿比对；未做"新建探针草稿"的写操作。
- 清理：删除本轮只读实测遗留的 2 个临时会话与 2 条假运行（`rewrite_job_id=job-fake-rewrite`，此前脚本超时未执行清理），复查在跑任务 0、样本草稿仍为 `(v4, pending_review, 2628 字)`。
...
- 用户指令：贴出审核记录（评分 58/0/74…）问“为什么改稿之后分数反而下降了？是不是规则不一致？”；核实答复后追加要求：“增加查看历史版本功能，在仅运行审核按钮旁边增加仅根据意见改稿按钮并和其他按钮一样适配”。四项全做（长度压力、记录标注、评分刻度与历史参照、历史版本 + 新按钮）。
- 核查结论（只读实测）：①**74 → 58 之间没有改稿**——中间两次版本变化都是 `source_regeneration`（按来源整篇重写）：v2 2047 字（09:59）→ v3 2245 字（10:04）→ v4 2483 字（12:02）；②分数下降是真的：v4 的 4-gram 重复占比 0.101 对 v2 的 0.052（约两倍），58 分那条点名的 24 个片段里 20 个确实在正文中；③**真因是长度压力**——提示词只说“别写到接近上限”，模型往上限堆并靠“换个说法重复”补字数；④记录页只显示分数，看不出审的是哪一版、那一版怎么来的。
- 规则一致性核查：审核模型 `temperature=0`（确定性，同一正文同分）；审核与改稿共用同一 `article_length_band`；但**跨次评分没有可比性**（每次独立、看不到上次分数，刻度只有“85–89＝可发布”一句）；另外 09:59 那次审核开始时 v2 最新、10:04 被 v3 替换，它审的是**已被替换的版本**（既有的“审核撞重写”竞态）。
- 处理结论：①生成/改稿提示词改为“**写到中段就好（大约 1900 字），不要往上限凑**……字数与信息量冲突时宁短勿凑”，并把“同一件事全文只许说一次、换说法重复同样算重复、写完逐段自检”写成硬规则；②审核提示词新增可复算刻度（100 起算，critical −25／major −8／minor −2，最低 20，同一处只扣一次）与**历史参照**（上次审的是第几版、多少分，并声明只评价当前这一版）；③审核报告写入 `reviewed_version` 与 `version_origin`，`list_auto_reviews` 返回版本、来源、是否为当前版与 `score_delta`；草稿侧记 `content_plan_json.last_revision_reason`；④生成记录页新增“历史版本”（来源/字数/时间，可展开看正文与当时意见，只读）；⑤发布页“仅运行审核”旁新增“仅根据意见改稿”（命令 `按审核意见改稿｜draft=…` → `apply_revision_issues`；无审核记录时给出下一步，版本对不上时拒绝）。
- 排错记录（实施中）：改稿的版本校验最初只看 `reviewed_version`，但**本轮之前的旧记录没有该字段**，于是退回时间比较 `review.created_at < draft.updated_at`；实测发现 `draft.updated_at` 会被“切当前文章”这类元数据更新顶掉（12:35 的会话内存更新把它推到最新），因此时间比较只能算近似——已在代码与测试里注明“宁可放过也不要误拒”。
- 验证（单测 + 只读实测，**未跑生成/审核/改稿**）：后端 **483 项通过、0 失败**（新增 12 项）；前端 `tsc` 0 错误；app/worker/frontend 已重建；容器内实测：模拟新记录的标注输出“评分 58 第 4 版（按来源重写）是当前版=True 上次 74 分数差 −16”“评分 74 第 2 版（按审核意见改稿）不是当前版”；历史版本三条（v4 按来源重写 2483 字 / v3 按来源重写 2245 字 / v2 按审核意见改稿 2047 字）；提示词关键句全部命中。
- 清理：实测探针通过工具创建了 2 条 `regenerate_draft` 运行（改稿入队被打桩、任务从未运行），已收尾为失败并写明“验证记录，未执行”，运行中归零。
- 未提交 git：`7f4eaf2`（10 文件，+321/−14）、`4f464ec`（前端 2 文件，+94/−3；App.tsx 只暂存本轮 5 个 hunk、styles.css 只暂存 1 个 hunk，并行会话改动未纳入）。
- 授权状态：已确认并完成

### 2026-09-15｜变更 333

- 用户指令：优化右侧 todo；核实目前状态；“正在处理”应明确显示为“正在处理”并带动态效果，其他状态按状态差异化显示。
- 影响范围：`frontend/src/App.tsx`（状态标签、清单分组与无障碍提示）、`frontend/src/styles.css`（六种状态的图标与运行态动画）、`app/services/session_tasks.py`（运行态统一文案）、`tests/test_session_tasks.py`（文案断言）；并追加计划、发现和进度记录。
- 处理结论：保留既有六态状态机 `pending → ready → running → completed / failed / skipped`；运行态由“进行中”统一为“正在处理”，以蓝色转环和轻微脉冲表示。等待/待处理/完成/失败/跳过均为静态标识；历史折叠从“已完成”改为“已结束”，避免把失败或跳过误称完成。
- 验证：会话任务单测 11 项通过（1 条既有第三方弃用警告）；前端 `npm --prefix frontend run build` 通过；`git diff --check` 通过。重新核验生成、审核、投递任务均为 0 后，仅重建 `frontend` 容器；页面入口 `http://127.0.0.1:5173` 返回 HTTP 200，未重启 app、worker 或数据服务。
- 授权状态：已确认

### 2026-09-15｜变更 334

- 用户指令：反馈右侧 todo “挤成一坨”，确认优化窄栏显示效果。
- 影响范围：`frontend/src/App.tsx`（任务项信息层级）、`frontend/src/styles.css`（窄栏网格与两行说明限制）；并追加计划、发现、进度和变更记录。
- 处理结论：状态图标独占 18px，任务名优先显示；状态改为任务名下方的小标签，依赖与说明分行。说明最多显示两行，完整内容保留在悬停提示；运行态动画继续只作用于状态图标。未改任何任务状态、依赖、入队或审核/投递逻辑。
- 验证：前端 `npm --prefix frontend run build` 通过；`git diff --check` 通过。生成、审核、投递任务均为 0 后，仅重建并替换 `frontend` 容器；页面入口 `http://127.0.0.1:5173` 返回 HTTP 200，未重启 app、worker 或数据服务。
- 授权状态：已确认

### 2026-09-15｜变更 335

- 用户指令：贴出重写失败截图（卡片显示“内容模型输出不符合草稿结构，请重试或检查模型配置”）要求“检查原因并查看证据包是否真的传入了”；核实后按用户口径处理：“字数只按配置页设置填充，不要因为字数问题重写，因为生成时间很长”。
- 核查结论（只读实测）：①**证据包没问题**——README 快照 1277 字经 `build_evidence` 全部进入提示词（`len(content) <= budget` 时原样返回，所以没有 `evidence_built` 日志，一开始会误判为“没传”），预算 20000 未触发裁剪；②真实原因是长度：`NaturalArticleError: 正文至少需要 1400 个字符（当前 1313）`；③而 1400 来自变更 332 为治“往上限堆字数”加的“写到中段、宁短勿凑”，**模型这次踩穿了下限**（1313 字，差 87）；④同时四个 `agent_skills/*-content-writing/SKILL.md` 写死“至少 1200 字符”，与代码的 1400 不一致——1313 正好落在两者之间的缝里；⑤草稿未被改动（仍 v4/2483 字，`updated_at` 停在昨天 12:35）。
- 影响范围：`app/services/generator.py`（成形下限、写作提示词、失败文案）、`app/tools/auto_review.py`（长度改为 minor 提示）、`app/tools/auto_revision.py`（重试触发条件与提示词）、四个来源 `SKILL.md` 与 `agent_skills/ARCHITECTURE.md`（字数改为“以配置页为准”）；新增 `tests/test_length_policy.py`。
- 处理结论（按用户口径：字数以配置页为准、不因为字数重写）：①成形阶段只兜绝对底线 `MIN_HARD_BODY_CHARS = 400`，低于配置下限只是“偏短”，**照常保存**；②提示词改为“区间来自配置页 / 下限是硬性的 / 不许为了凑字数重复 / 偏短偏长都会照常保存，不会因此重写”；③改稿的长度修复重试只在低于 400 时触发（原先按 1400 触发 = 为几十个字重跑一次模型）；④规则审核不再把长度计入 `failures`，改为 `length_note` 作为 minor 提示并入意见，审核提示词明确“不得因为长度给 major/critical、不得要求重写整篇”；⑤失败文案按真因分类：`模型写得太短：正文至少需要 400 个字符（当前 1313）…已保留原稿、未覆盖`。
- 排错记录：本轮 `edit` 工具遇到 `ReplaceFileW EIO (Win32 1175)` 写失败两次（Windows 文件占用），短暂等待后重试成功；`generator.py` 中新引用的 `NaturalArticleError` 一开始没加进 import，已补。
- 验证（单测 + 只读实测，**未跑模型**）：后端 **492 项通过、0 失败**（新增 8 项策略测试 + 更新 12 项旧断言）；app/worker 已重建（重建前在跑任务 0/0）；容器内实测：1312/900/500 字**都通过并保存**，380 字被拒且提示“至少需要 400 个字符（当前 380）”；规则审核对偏短正文 `passed=True`、`failures=[]`、只给一条 minor 长度提示；失败文案输出“模型写得太短…已保留原稿、未覆盖”。
- 未提交 git：`742e5f5` 已提交（11 文件，+236/−73）。
- 授权状态：已确认

### 2026-09-15｜变更 336

- 用户指令：优化页面显示逻辑——将长文页操作按钮前置；模型测试结果显示在对应模型行并优化；新增一键测试全部模型；草稿箱相关内容移动到更合适位置。
- 影响范围：`frontend/src/App.tsx`（模型测试前端状态、顺序式全量测试、草稿箱准备标识）、`frontend/src/styles.css`（标题下操作栏、模型测试行内状态、发布流程排序）；并追加计划、发现、进度和变更记录。
- 处理结论：文章操作栏固定在标题下方；单模型与全量测试均复用既有脱敏检测接口，测试结果仅在当前设置页会话显示，全量测试按顺序继续到全部结束；当前文章的草稿箱准备前置到发布流程区，远端草稿箱保持底部只读同步。没有改动模型档案、API Key、审核、投递或状态机逻辑；实施过程不调用模型。
- 验证：前端 `npm --prefix frontend run build` 通过；`git diff --check` 通过。生成、审核、投递任务均为 0 后，仅重建并替换 `frontend` 容器；页面入口 `http://127.0.0.1:5173` 返回 HTTP 200，未重启 app、worker 或数据服务。
- 授权状态：已确认

### 2026-09-15｜变更 337

- 用户指令：贴出 50 → 60 分两轮审核记录问“为什么改稿后仍然不合格？”；确认方案 A+B 治根，并追加口径：“材料不足时调用搜索工具补充啊，搜索背景、应用什么的”。
- 核查结论（只读实测）：①60 分那条审核点名的 **5 句全部真实存在于正文**（逐句核对 5/5），不是误判；②按段扫描发现 8 段里 **6 段**是同一个模板——“先列举，再补一句以『这个路径/这个目录/这个样例/它/这些方向』开头的复述”；③根因是**材料不足**：该草稿只有 1277 字 README，却要写 1600–2200 字，而**生成/重写路径从不联网补充**（搜索只在“审核后自动改稿”那一步用；`EnhancedDraftGenerator` 的搜索默认关闭且当前生成器不是它），模型只能用重复填充；④改稿只治标：提示词写着“其余部分尽量保留原文表述”，模型只删被点名的句子，同一模板在新位置继续存在，于是复审又报 5 条 major（5×8 分 ≈ 60 分）；⑤字数已按上一轮口径落到 1814 字，落在配置区间内。
- 影响范围：`app/services/generator.py`（材料不足时的联网补充、提示词禁令）、`app/services/search_evidence.py`（新增：联网段落格式与规则文本，生成/改稿共用）、`app/tools/auto_revision.py`（改用共用段落、点名清“指代式复述”、放宽“保留原文”）；新增 `tests/test_supplemental_search.py`。
- 处理结论：①**材料不足自动联网补充**——可用材料短于目标中段（`SEARCH_TRIGGER_RATIO=0.9`）时，用“<项目名> 是什么 背景 用途”检索一次，结果作为“联网补充资料”段落进提示词，并写明用途边界（只补背景与名称说明、不得编造项目事实）；检索失败只记日志、不影响生成；②**点名禁止“指代式复述”**（生成与改稿提示词都加，并给出 `.codex-plugin` 那一对可直接照做的反例）；③**放宽改稿的“保留原文”**：明确允许删句、段内改写、合并句子（不动事实与段落结构），并要求把**未被点名的同类句子**一并处理。
- 排错记录：`_supplemental_search` 最初用 `draft_body_max_chars` 当基准（日志里 `target_mid=2200`）导致阈值偏高，且第二条检索词会从 README 标题里抓到「SwiftUI」这类无关词；已改为目标中段（1900 → 阈值 1710）并只用“项目名 + 是什么/背景/用途”一条检索词。`build_generator()` 返回的是 `ResilientDraftGenerator`（探针一开始在包装层取 `_supplemental_search` 报 AttributeError，改从 `.primary` 取）。
- 验证（单测 + 只读实测，**未跑内容模型、未外呼 Exa**）：后端 **499 项通过、0 失败**（新增 7 项）；app/worker 已重建（重建前在跑任务 0/0）；容器内实测该草稿：可用材料 1277 字 < 阈值 1710 字 → **触发联网补充**，检索词 `openai/plugins 是什么 背景 用途`，取回 1 条并组装成带边界的提示词段落；生成提示词含“指代式复述”禁令与反例。
