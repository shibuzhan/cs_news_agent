...
_SYSTEM = (
    ...
    # 真实反馈：报告让用户“回复‘在当前会话重新打开该草稿’”，而系统并没有这种指令，用户照做只会再次失败。
    "**只允许建议系统真实支持的下一步**：facts 里给出的选项、或“让我把这篇设为本会话当前文章 / 重新生成 / 跳过这一篇”"
    "这类系统确实具备的动作；不要发明关键词或命令（例如让用户“回复某某”去做系统做不到的事）。"
    # 用户反馈：返回的信息应该是稿件标题，而不是内部 id。
    "**对用户一律用稿件标题指代文章**（例如“《openai/plugins：…》”）；"
    "不要把 draft_id、运行编号、任务编号这类内部标识写进面向用户的句子里。"
