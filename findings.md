...
## 2026-09-14｜编辑台式前端重构基线

- 对话页已具备“会话 / 消息 / 附件”三栏结构，但样式来自多轮追加规则：默认消息区存在 `max-height:540px`，而外层三栏已使用更高的对齐高度，导致视觉上中间消息区和两侧不连续。
- 生成记录中仍挂载 `WechatDraftPreparation`；发布情况同时拥有审核、投递、状态与预览，因此同一篇文章存在两个公众号草稿箱操作入口。迁移仅调整前端位置，继续复用既有 API 与确认弹窗。
- 自动审核通过后投递公众号草稿箱是用户确认保留的行为；本轮不改变自动审核开关、运行设置、接口请求或任何相关文案。
- 模型档案的 API Key 已使用固定掩码，视觉问题来自行项目使用弹性布局；应使用“模型 / 掩码 / 操作”稳定列，而不是改变密钥处理方式。

## 2026-09-15｜会话任务清单状态可视化

- 右侧清单使用的是持久化的 `SessionTask`：状态机为 `pending → ready → running → completed / failed / skipped`，不应为了视觉效果新增或改写状态值。
- 原页面把 `running` 显示为“进行中”且只有静态圆点；完成、失败、跳过又统一归在“已完成”折叠区，状态含义被弱化。前端应以 `running` 为唯一动态焦点，并把历史区改为“已结束”。

## 2026-09-15｜右侧清单窄栏排版

- 右侧工作抽屉宽度有限，状态组件原先以 `min-width: 74px` 与任务说明同排，长说明只剩半行宽而被拆成碎行。状态文字与说明不应争抢同一列。
- 调整为 18px 图标列 + 内容列：任务名优先，状态改为下方小标签，说明独占内容列并限制两行；完整说明仍以 `title` 保留，避免删除审计信息。

## 2026-09-15｜高频操作与模型测试可见性

- 生成记录的文章操作原本处在证据、审核历史和版本快照之后；长文页面必须滚到底才能操作。将它作为标题后的粘性操作栏，不改原有按钮条件或确认弹窗。
- 单模型检测接口已经返回脱敏的成功/失败结论，但旧界面只把成功写到全局提示。测试结果应归属到对应模型行；“测试全部”前端按顺序调用既有接口，避免同时向全部模型服务发请求，且只在用户点击时执行。
- 本篇草稿箱准备和远端草稿箱同步属于不同层级：前者是当前文章的发布步骤，后者是只读盘点。前者应前置到发布流程区，后者继续留在页面下方。

## 2026-09-15｜Todo 收尾与窗口自适应

- `chat_dispatch` 已只在 `plan.multi` 且步骤数不少于两步时派发计划清单；此前确定性按钮命令仍会绕过这条规则，分别创建一条孤立 Todo。
- `SessionTaskList` 把所有终态任务收进历史区，导致每一轮已完成事项长期占用右侧。正确的工作台语义是：开放态和失败态可见，完成/跳过由生成、审核、发布记录承接。
- 原对话区在后置 CSS 覆盖后，`900px` 以下仍保留中间列最小宽度；发布区在侧栏仍存在的中等窗口也可能保留两列最小值。必须根据实际可用工作区宽度提前折叠，而不是仅在手机宽度切换。

## 2026-09-15｜审核模型结构化检索计划

- `ReviewResponse` 已由 Pydantic 约束并经审核模型的既有一次调用返回；`AutoReviewTool` 将其保存到 JSON 类型的 `model_report_json`，可新增 `search_plan` 而无需迁移表结构。
- 改稿检索目前只消费字符串 `search_queries`；应优先消费结构化计划并由代码构造最终查询，同时兼容历史字符串记录。初次生成没有审核步骤，不额外增加规划模型调用。
- 当前名称提取会把 `affaan-m/ECC` 降成 `ECC`；应先识别完整仓库标识。`Codex 是什么` 又会被现有方面提示错误视为可执行查询，必须在计划校验中拒绝。
- 实现后模型只能提出计划，不能直接执行查询：`subject` 与 `need` 必须同时存在，服务端拒绝通用主体和空泛意图，并依据来源偏好追加“官方文档”或“README”。这保留模型的内容判断，也保留后端的检索安全边界。
- 运行镜像缺少 pytest，宿主解释器缺少 `deepagents`；将当前源码只读挂载到已有依赖环境后，结构化计划、拒绝规则、仓库标识、计划优先和旧字段兼容的断言均通过。镜像构建仍因 PyPI TLS EOF 未完成，不能发布新代码。
- 会话 Agent 的 `search_web_evidence` 原描述只接收裸 `queries`，会让模型绕开“主体/缺口/理由”的判断；普通生成和增强生成也各有不同的规划规则。统一后，工具和两种生成模式都以计划为入口，旧字符串只作历史兼容。
- 新审核记录显式返回空 `search_plan` 时，改稿不再从正文抽名称搜索；否则“模型决定不搜”会被确定性兜底推翻。缺少该字段的历史审核记录才保留旧字符串和正文兜底。

## 2026-09-15｜编辑区顶部操作与审核摘要

- 生成记录页的操作原本位于正文、证据、审核历史和版本快照之后，长文必须滚到底才能保存、重写或审核。标题和操作现在共享同一行粘性工作条；760px 以下改为标题在上、操作在下，避免窄屏硬挤。
- 发布页的审核操作属于“发起动作”，不应跟在审核历史之后。页面保留同一 DOM 与接口，只用视觉顺序将动作置于审核记录摘要前，草稿箱准备仍随后展示。
- 审核记录默认只呈现分数、与上次的分差、critical 结论和模型摘要；可展开的“查看具体问题（N）”保留完整问题，不丢失审计内容。
- 数据结构兼容旧字符串问题项和新结构化问题项；只有结构化 `severity=critical` 才计入 critical 数，旧记录无该字段时按“无 critical”显示而不推断。
